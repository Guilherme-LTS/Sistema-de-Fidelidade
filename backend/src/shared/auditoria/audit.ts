...
export async function logAuditEvent(input: AuditLogInput): Promise<QueryResult | null> {
  const {
    req,
    tenantId,
    action,
  ...
  try {
    ...
    return await db.query(queryWithBusinessFields, values);
  } catch (error: any) {
    if (error?.code === '42703') {
      if (client) {
        return await client.query(queryLegacy, values);
      }
      return await db.query(queryLegacy, values);
    }
