...
router.get('/:cpf', async (req: Request, res: Response) => {
  try {
    const cpfParam = (req.params.cpf as string).replace(/\D/g, '');
    ...
    const proximoVencimentoResult = await db.query(
      `SELECT MIN(data_vencimento) as proximo_vencimento FROM transacoes WHERE cliente_id = $1 AND data_vencimento > NOW() AND data_liberacao <= NOW() AND pontos_restantes > 0`,
      [clienteId]
    );
    const proximoVencimento = proximoVencimentoResult.rows[0].proximo_vencimento;

    const expiracaoUrgenteResult = await db.query(
      `SELECT COALESCE(SUM(pontos_restantes), 0) as pontos_expirando, MIN(data_vencimento) as data_proxima_expiracao FROM transacoes WHERE cliente_id = $1 AND data_liberacao <= NOW() AND data_vencimento > NOW() AND data_vencimento <= NOW() + INTERVAL '7 days' AND pontos_restantes > 0`,
      [clienteId]
    );
    ...
    const liberacaoUrgenteResult = await db.query(
      `SELECT MIN(data_liberacao) as data_proxima_liberacao FROM transacoes WHERE cliente_id = $1 AND data_liberacao > NOW() AND pontos_restantes > 0`,
      [clienteId]
    );
...
router.get('/:cpf/extrato', async (req: Request, res: Response) => {
  try {
    const cpfParam = (req.params.cpf as string).replace(/\D/g, '');
    if (!cpfParam || cpfParam.length !== 11) {
      return res.status(400).json({ error: 'Formato de CPF inválido.' });
