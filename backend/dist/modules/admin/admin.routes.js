...
router.get('/auditoria', autenticacao_1.default, async (req, res) => {
    ...
    try {
        ...
        try {
            const tenantCountResult = await (0, db_rls_1.queryWithRLS)(authReq, `
          SELECT COUNT(*)::int AS count
          FROM audit_logs a
          WHERE a.tenant_id = $1
        `, [tenantId]);
            totalAuditLogsTenant = tenantCountResult.rows[0]?.count || 0;
            const countResult = await (0, db_rls_1.queryWithRLS)(authReq, `
          SELECT COUNT(*)::int AS count
          FROM audit_logs a
        ...
        if (hasAuditTable && totalAuditLogsTenant > 0) {
            const result = await (0, db_rls_1.queryWithRLS)(authReq, `
          SELECT
            a.id,
        ...
        `, [...params, safeLimit, offset]);
            const summaryResult = await (0, db_rls_1.queryWithRLS)(authReq, `
          SELECT
            COUNT(*)::int AS total,
        ...
        const legacyParams = [safeLimit, offset, tenantId];
        ...
        const legacyResult = await (0, db_rls_1.queryWithRLS)(authReq, `
        SELECT *
        FROM (
          SELECT
            'tx-' || t.id::text AS id,
        ...
        LIMIT $1 OFFSET $2
      `, legacyParams);
        const legacyCount = await (0, db_rls_1.queryWithRLS)(authReq, `
        SELECT COUNT(*)::int AS count
        FROM (
          SELECT
            t.created_at AS data_hora,
