...
router.post('/', autenticacao_1.default, async (req, res) => {
    ...
    try {
        ...
        await (0, db_rls_1.withRlsTransaction)(authReq, async (client) => {
            const configs = await (0, pontos_service_1.loadTenantPointSettings)(client, tenantId);
            const { availableAt, expiresAt } = (0, pontos_service_1.calculatePointTimelines)(configs.carencia_pontos, configs.expiracao_pontos);
        ...
        return res.status(500).json({ error: 'Ocorreu um erro no servidor.', details: error.message });
    }
});
