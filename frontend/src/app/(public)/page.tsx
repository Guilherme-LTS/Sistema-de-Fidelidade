...
const planOptions = {
  mensal: { price: "139,90", suffix: "/mês", saving: "", label: "Cobrança mensal" },
  anual: { price: "1.299,00", suffix: "/ano", saving: "Economize R$ 379,80 no ano", label: "Cobrança anual" },
}

const chartData = [42, 58, 48, 72, 64, 86, 96]

export default function LandingPage() {
  const [billing, setBilling] = useState<keyof typeof planOptions>("mensal")
  const selectedPlan = planOptions[billing]
  ...
  return (
    <div className="min-h-screen bg-background font-sans text-foreground">
      ...
      <main>
          ...
          <div className="mx-auto grid max-w-7xl items-center gap-14 lg:grid-cols-[1.02fr_.98fr] lg:gap-20">
            ...
            <div className="relative mx-auto w-full max-w-[520px]">
              ...
              <div className="relative overflow-hidden rounded-[1.5rem] border border-primary/10 bg-white p-3 shadow-2xl shadow-primary/15 sm:p-5">
                ...
                <div className="rounded-xl border border-border/70 p-4"><div className="mb-4 flex items-center justify-between"><p className="text-sm font-semibold">Clientes recorrentes</p><span className="text-xs text-primary">Últimos 7 dias</span></div><div className="flex h-28 items-end gap-2">{chartData.map((height, i) => <div key={i} className="flex flex-1 flex-col items-center gap-2"><div className="w-full rounded-t-md bg-primary/80 transition-all hover:bg-accent" style={{ height: `${height}px` }} /><span className="text-[10px] text-muted-foreground">{["S", "T", "Q", "Q", "S", "S", "D"][i]}</span></div>)}</div></div>
        ...
        <section className="border-y border-border/70 bg-white px-5 py-7 lg:px-8"><div className="mx-auto flex max-w-5xl flex-col items-center justify-between gap-4 text-center sm:flex-row sm:text-left"><div><p className="text-sm font-semibold text-primary">Feito para a rotina do balcão</p><p className="mt-1 text-sm text-muted-foreground">Uma experiência simples para bares, sorveterias, lanchonetes e cafeterias.</p></div><div className="flex items-center gap-2 rounded-full bg-[#fff0df] px-4 py-2 text-sm font-semibold text-[#a34f12]"><div className="h-2 w-2 rounded-full bg-accent" /> Comece sem complicação</div></div></section>
        ...
        <section id="planos" className="bg-[#f4faf6] px-5 py-24 lg:px-8"><div className="mx-auto max-w-5xl"><div className="max-w-2xl"><p className="text-sm font-semibold text-accent">Um plano para começar bem</p><h2 className="mt-3 text-3xl font-bold tracking-tight sm:text-4xl">Tudo o que o seu negócio precisa para criar hábito.</h2><p className="mt-4 text-lg leading-8 text-muted-foreground">O Pontus Pro reúne as ferramentas essenciais em uma assinatura clara, sem empurrar você para planos diferentes.</p></div><div className="mt-12 grid gap-8 border-t-2 border-[#0d1f16] pt-8 lg:grid-cols-[1fr_1.1fr] lg:items-start"><div><p className="text-3xl font-bold">Pontus Pro</p><p className="mt-3 max-w-sm leading-7 text-muted-foreground">Acesso total a todas as funcionalidades do sistema para alavancar suas vendas.</p><ul className="mt-7 grid gap-3 sm:grid-cols-2">{["Programa de pontos", "Recompensas personalizadas", "Relatórios de desempenho", "Clientes ilimitados", "Múltiplos operadores", "Suporte prioritário"].map(item=><li key={item} className="flex gap-2 text-sm"><Check className="h-4 w-4 shrink-0 text-primary" />{item}</li>)}</ul></div><div className="border-l-0 border-primary/20 lg:border-l lg:pl-10"><div className="inline-flex rounded-full border border-primary/20 bg-white p-1" role="group" aria-label="Periodicidade da cobrança"><button type="button" onClick={() => setBilling("mensal")} aria-pressed={billing === "mensal"} className={`rounded-full px-4 py-2 text-sm font-semibold transition-colors ${billing === "mensal" ? "bg-[#0d1f16] text-white" : "text-muted-foreground"}`}>Mensal</button><button type="button" onClick={() => setBilling("anual")} aria-pressed={billing === "anual"} className={`rounded-full px-4 py-2 text-sm font-semibold transition-colors ${billing === "anual" ? "bg-[#0d1f16] text-white" : "text-muted-foreground"}`}>Anual</button></div><p className="mt-7 text-sm text-muted-foreground">{selectedPlan.label}</p><div className="mt-1 flex items-baseline gap-2"><span className="text-sm text-muted-foreground">R$</span><span className="text-5xl font-bold tracking-tight">{selectedPlan.price}</span><span className="text-sm text-muted-foreground">{selectedPlan.suffix}</span></div>{selectedPlan.saving && <p className="mt-2 text-sm font-semibold text-primary">{selectedPlan.saving}</p>}<Button asChild className="mt-7 rounded-full px-7"><a href={appSignup}>Começar agora</a></Button></div></div></div></section>

        <section className="px-5 py-24 lg:px-8"><div className="mx-auto grid max-w-5xl gap-10 border-l-4 border-accent bg-[#0d1f16] px-7 py-10 text-white sm:px-12 lg:grid-cols-[1fr_auto] lg:items-center"><div><p className="text-sm font-semibold text-orange-200">Uma ideia para o seu balcão</p><blockquote className="mt-4 max-w-2xl font-heading text-2xl font-semibold leading-snug sm:text-3xl">“Meus clientes adoram ganhar uma casquinha depois de algumas visitas. O Pontus deixou isso fácil de explicar e de acompanhar.”</blockquote><p className="mt-5 text-sm text-white/70">Marina Alves · Sorveteria Ponto Gelado</p><p className="mt-2 text-xs text-orange-200/80">Conteúdo placeholder — substituir pelo depoimento real</p></div><div className="hidden h-24 w-24 items-center justify-center rounded-full bg-accent/20 lg:flex"><Store className="h-10 w-10 text-orange-200" /></div></div></section>
